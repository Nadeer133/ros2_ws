...
public:
    ...
    {
        this->declare_parameter("led_states",std::vector<int64_t>{100,100,100});
        this->declare_parameter("led_On/Off",0);
        ...
        timer_=this->create_wall_timer((1s), std::bind(&LedPanelNode::ledpanelaction, this));
    }
private:
    void ledpanelaction()
    {
        auto request_= std::make_shared<my_ros2_interfaces::srv::LedState::Request>();
        my_ros2_interfaces::msg::StateLed msg;
        request_->ledstate=this->get_parameter("led_On/Off").as_int();
        if(request_->ledstate==1)
        {
            //response_->status = "Warning LED Turned ON";
            led_panel=this->get_parameter("led_states").as_integer_array();
            RCLCPP_INFO(this->get_logger(),"[%ld,%ld,%ld]",led_panel[0],led_panel[1],led_panel[2]);
        ...
        else
        {
            ...
            msg.msg="led: %ld" + std::to_string(request_->ledno) +" is turned OFF";
            RCLCPP_INFO(this->get_logger(),"[0,0,0]");
            msg.msg="led is turned OFF, [0,0,0]";
            publisher_->publish(msg);
        }
    }
std::vector<int64_t> led_panel={0,0,0};
