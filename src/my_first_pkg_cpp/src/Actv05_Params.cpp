...
public:
    ...
    {
        this->declare_parameter("Robot_name","R0D0");
        publisher_ = this->create_publisher<example_interfaces::msg::String>("robot_news_ChannelX", 10); // creating the actual publisher using create_publisher API. And also the API works only with objects. this-> points to the current object of RobotNewsStation class.
...
private:
    ...
    msg.data = std::string("Hi, this is ") + this->get_parameter("Robot_name").as_string() + std::string(" from the robot news station.");
